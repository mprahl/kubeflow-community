# KEP-NNNN: Centralize Experiment Tracking in Kubeflow
...
to reliance on the inactive MLMD (Machine Learning Metadata) project.
...
1. **Maintenance challenges**: The Kubeflow Pipelines dependency on MLMD (Machine Learning Metadata) creates technical
   debt and limits future flexibility due to its lack of active maintenance. MLMD also overlaps with the Model Registry
...
1. **Kubeflow centralized experiment tracking**: Create an experiment tracking system that is independent from Kubeflow
   Pipelines, enabling experiments to be logged and managed without requiring pipeline execution.
...
1. **Remove the MLMD dependency from Pipelines**: Decouple Kubeflow Pipelines from MLMD (Machine Learning Metadata) to
   remove a core dependency on a project outside of the Kubeflow ecosystem.
...
1. **Provide MLFlow SDK compatibility**: Create an
   ...
   familiar MLFlow SDK experiment tracking APIs while working with the centralized Kubeflow experiment tracking
   solution.
...
The UI will provide a comprehensive interface for experiment management and visualization with feature parity to the
existing Kubeflow Pipelines run comparison functionality, while extending support to all run sources generically.
...
#### Story 1: Data Scientist Logging Experiments from Jupyter Notebooks
...
Kubeflow dashboard alongside pipeline runs and other ML workflows.
...
1. **Migration Challenges**: The proposal involves migrating away from MLMD (Machine Learning Metadata), which is
   currently used by Kubeflow Pipelines. This migration process could be complex and risky, potentially breaking
   existing workflows. To mitigate this risk, we will implement extensive testing and maintain an extended transition
   period where Kubeflow Pipelines registers to both MLMD and Model Registry.
...
1. **Experiments**: A group of related runs. This would replace experiments in Pipelines and could map to experiments in
   Katib.
1. **Users**: A user to associate runs, metrics, artifacts, and etc. for auditing. This will generally map to the
   Kubernetes identity.
...
Registry would address this gap for all of Kubeflow's metadata needs.
...
example, when a user calls "/api/model_registry/v1alpha3/registered_models/1" to access a registered model named
"my-model" in the namespace "my-project", the system would make a subject access review call on the following resource:
...
rules:
  - apiGroups: ["modelregistry.kubeflow.org"]
    resources: ["registered-models"]
    verbs: ["get", "list"]
    resourceNames: ["my-model"] # Optional: restrict to specific model
```
...
This integration should feel cohesive with the existing Model Registry SDK functionality for registering models.
